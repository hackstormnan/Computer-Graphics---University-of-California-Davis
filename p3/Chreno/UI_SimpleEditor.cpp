...
void UI_SimpleEditor::Render()
{
	...
	ImGui::Checkbox("Halftoned On", &Engine::Get().m_halfToneOn);
	ImGui::Checkbox("Using Phong", &m_usingPhong);
	ImGui::Text("If not using Phong then use Gouraud");

	ImGui::Checkbox("Using Painter's Algorithm", &m_usingPainter);
	if (m_usingPhong != m_usingPhongLastFrame) {
		for (unsigned int i = 0; i < m_entities.size(); i++)
		{
			EntityInstance * instance = dynamic_cast<EntityInstance*>(m_entities[i]);
			if (instance != nullptr) {
				if (m_usingPhong) {
					instance->UpdateShader("Assets/Shaders/Basic.shader");
				}
				else {
					instance->UpdateShader("Assets/Shaders/Gouraud.shader");
				}
			}
		}
		m_usingPhongLastFrame = m_usingPhong;
	}

	if (m_usingPainter != m_usingPainterLastFrame) {
		for (unsigned int i = 0; i < m_entities.size(); i++)
		{
			EntityInstance* instance = dynamic_cast<EntityInstance*>(m_entities[i]);
			if (instance != nullptr) {
				if (m_usingPainter == false) {
					instance->UpdateDrawType(DrawType::DEFAULT);
				}
				else {
					instance->UpdateDrawType(DrawType::PAINTERS_ALGORITHEM);
				}
			}
		}
		m_usingPainterLastFrame = m_usingPainter;
	}
